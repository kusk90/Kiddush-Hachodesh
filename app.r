...
d1 = as.Date('3/29/1178',format='%m/%d/%Y')
...
ui <- fluidPage(
  dateInput(inputId = "khdate", label = "Enter Date", value = '1178-04-27',format='mm/dd/yyyy'),
  mainPanel(plotOutput("chart"),verbatimTextOutput("pos"))
)

server <- function(input, output) {data = reactive({   
  d1 = as.Date('3/29/1178',format='%m/%d/%Y')
  d2 = as.Date(input$khdate,format='%m/%d/%Y')
...
maslul_lat = if_else(maslul_lat < 0, maslul_lat + 360, maslul_lat)
...
if_else(dist2 < 64,paste0("sun position: ", true_sun[1], " degrees, moon position: ", true_moon[1],
      " degrees, kitzei hariyah: ", vis_points[1], ", oirech rishon: ", long1[1],
      ", Visible? ", vis3),
      paste0("sun position: ", true_sun[1], " degrees, moon position:", true_moon[1],
             " degrees, merchak hakaful: ", dist2, " - not night of reiyah")
)
})
...
output$chart = renderPlot({
  ...
  circos.trackPoints(factor = 1, x=as.numeric(data()), y=5, col = "yellow", pch = 16, cex = 5)
  legend(-0.4,0.6,
         legend=c("sun","moon","apogee","epicycle","head","tail"), 
         col=c("Yellow","Blue","black","Blue","green","red"),
         pch=c(16,21,16,21,16,16), pt.cex = c(5,3,1,1,1,1), bty = 'n', cex=2)
})
output$pos <- renderText({ data() })  
